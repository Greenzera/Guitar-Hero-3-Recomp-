@echo off
REM Build experimental contra o ramo v0.8.1.19 do SDK (o do Skate 3), que traz o
...
set BDIR=out/build/win-amd64-081
set JOBS=%1
if "%JOBS%"=="" set JOBS=4
...
echo [configure 081] inicio %TIME%
cmake -S . -B %BDIR% -G Ninja ^
  ...
  -DCMAKE_C_FLAGS="-msse4.1" -DCMAKE_CXX_FLAGS="-msse4.1" > build081.log 2>&1
if errorlevel 1 (
  echo [configure 081] FALHOU - ultimas linhas:
  powershell -NoProfile -Command "Get-Content build081.log -Tail 25"
  exit /b 1
)
echo [configure 081] ok %TIME%

echo [build 081] inicio %TIME%  jobs=%JOBS%
cmake --build %BDIR% --target gh3 --parallel %JOBS% >> build081.log 2>&1
set RC=%errorlevel%
echo [build 081] fim %TIME%  rc=%RC%
exit /b %RC%
