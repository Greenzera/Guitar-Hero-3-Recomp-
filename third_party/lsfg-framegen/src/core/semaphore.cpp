...
Semaphore::Semaphore(const Core::Device& device, void* win32Handle) {
    // Ao importar nao se declara export info: o semaforo e' criado vazio e a
    // importacao substitui a sua payload pela do HANDLE.
    const VkSemaphoreCreateInfo desc{
        .sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO
    };
    VkSemaphore semaphoreHandle{};
    ...
    if (win32Handle != nullptr) {
        auto pfnImport = reinterpret_cast<PFN_vkImportSemaphoreWin32HandleKHR>(
            vkGetDeviceProcAddr(device.handle(), "vkImportSemaphoreWin32HandleKHR"));
        if (pfnImport == nullptr) {
            ...
            throw LSFG::vulkan_error(VK_ERROR_EXTENSION_NOT_PRESENT,
                "vkImportSemaphoreWin32HandleKHR unavailable");
        }
        ...
        res = pfnImport(device.handle(), &importInfo);
        if (res != VK_SUCCESS) {
            vkDestroySemaphore(device.handle(), semaphoreHandle, nullptr);
            throw LSFG::vulkan_error(res, "Unable to import semaphore from Win32 handle");
        }
    }
