...
#if defined(_WIN32)
// Variante Windows do construtor de import: identica a de fd, mas usando
// VkImportMemoryWin32HandleInfoKHR / HANDLE_TYPE_OPAQUE_WIN32 em vez de fd.
// win32Handle == nullptr => aloca memoria propria (sem import).
//
// Nota de propriedade do handle: ao contrario do fd (que o Vulkan consome), o
// driver NAO fecha o HANDLE importado -- quem chama continua dono e deve
// fecha-lo (CloseHandle) depois de a memoria estar ligada.
Image::Image(const Core::Device& device, VkExtent2D extent, VkFormat format,
        VkImageUsageFlags usage, VkImageAspectFlags aspectFlags, void* win32Handle)
        : extent(extent), format(format), aspectFlags(aspectFlags),
          externalShared(win32Handle != nullptr) {
    const VkExternalMemoryImageCreateInfo externalInfo{
        .sType = VK_STRUCTURE_TYPE_EXTERNAL_MEMORY_IMAGE_CREATE_INFO,
    ...
    const VkImageCreateInfo desc{
        .sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO,
        .pNext = win32Handle ? &externalInfo : nullptr,
        .imageType = VK_IMAGE_TYPE_2D,
        .format = format,
    ...
    auto res = vkCreateImage(device.handle(), &desc, nullptr, &imageHandle);
    if (res != VK_SUCCESS || imageHandle == VK_NULL_HANDLE)
        throw LSFG::vulkan_error(res, "Failed to create Vulkan image (win32)");
    ...
    if (!memType.has_value())
        throw LSFG::vulkan_error(VK_ERROR_UNKNOWN, "Unable to find memory type for image (win32)");
    ...
    const VkImportMemoryWin32HandleInfoKHR importInfo{
        ...
        .handle = static_cast<HANDLE>(win32Handle)
    };
    const VkMemoryAllocateInfo allocInfo{
        .sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
        .pNext = win32Handle ? static_cast<const void*>(&importInfo)
    ...
    res = vkAllocateMemory(device.handle(), &allocInfo, nullptr, &memoryHandle);
    if (res != VK_SUCCESS || memoryHandle == VK_NULL_HANDLE)
        throw LSFG::vulkan_error(res, "Failed to allocate memory for Vulkan image (win32)");

    res = vkBindImageMemory(device.handle(), imageHandle, memoryHandle, 0);
    if (res != VK_SUCCESS)
        throw LSFG::vulkan_error(res, "Failed to bind memory to Vulkan image (win32)");
    ...
    res = vkCreateImageView(device.handle(), &viewDesc, nullptr, &viewHandle);
    if (res != VK_SUCCESS || viewHandle == VK_NULL_HANDLE)
        throw LSFG::vulkan_error(res, "Failed to create image view (win32)");
    ...
    this->view = std::shared_ptr<VkImageView>(new VkImageView(viewHandle),
        [dev = device.handle()](VkImageView* v) { vkDestroyImageView(dev, *v, nullptr); });
}
#endif  // _WIN32
