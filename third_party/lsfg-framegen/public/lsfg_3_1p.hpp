...
#if defined(_WIN32)
    ///
    /// Variante Windows do createContext: as imagens chegam como HANDLEs
    /// opacos Win32 (VK_KHR_external_memory_win32) em vez de file descriptors,
    /// que nao existem no Windows.
    ///
    /// @param in0 HANDLE da primeira imagem de entrada.
    /// @param in1 HANDLE da segunda imagem de entrada.
    /// @param outN HANDLE de cada imagem de saida; define o nivel do LSFG.
    ///
    int32_t createContextFromWin32(
        void* in0, void* in1, const std::vector<void*>& outN,
        VkExtent2D extent, VkFormat format);

    ///
    /// Variante Windows do presentContext: semaforos como HANDLEs opacos.
    ///
    void presentContextWin32(int32_t id, void* inSem, const std::vector<void*>& outSem);
#endif
